...
# Желаемый порядок ключей в каждом сообщении
KEY_ORDER = [
...
def get_video_duration_ffprobe(fp: pathlib.Path):
    ...
    if not exe:
        return None
    # Сначала пробуем через stream
    cmd = [exe, "-v", "error", "-select_streams", "v:0",
           "-show_entries", "stream=duration", "-of", "json", str(fp)]
    ...
    try:
        ...
        pass
    # Если не сработало — через format
...
def find_nearest_date(div):
    # Пробуем взять дату из самого сообщения
    d = div.find("div", class_="pull_right date details")
    if d and d.has_attr("title"):
        dt_obj = datetime.strptime(d["title"], "%d.%m.%Y %H:%M:%S UTC%z")
    ...
    while prev:
        ...
        if d_prev and d_prev.has_attr("title"):
            dt_obj = datetime.strptime(d_prev["title"], "%d.%m.%Y %H:%M:%S UTC%z")
            return dt_obj.strftime("%Y-%m-%dT%H:%M:%S"), str(int(dt_obj.replace(tzinfo=None).timestamp()))
        prev = prev.find_previous_sibling("div", class_="message")
    # Ищем в следующих (вниз по DOM)
    ...
    while next_:
        ...
        if d_next and d_next.has_attr("title"):
            dt_obj = datetime.strptime(d_next["title"], "%d.%m.%Y %H:%M:%S UTC%z")
            return dt_obj.strftime("%Y-%m-%dT%H:%M:%S"), str(int(dt_obj.replace(tzinfo=None).timestamp()))
        next_ = next_.find_next_sibling("div", class_="message")
    return "", ""


def probe_format_duration(path: pathlib.Path):
    exe = shutil.which("ffprobe")
    if not exe:
        return None
    cmd = [exe, "-v", "error", "-show_entries", "format=duration",
...
def div_sticker_emoji(fp: pathlib.Path):
    # TODO: заменить на реальное чтение emoji из HTML-дерева
    return "❤️"
...
def get_file_info(fp: pathlib.Path, export_dir: pathlib.Path):
    ...
    suf = fp.suffix.lower()
    # "thumbnail" — ищем файл вида <stem>_thumb*<suffix>
    ...
    # Особая обработка для папки photos
...
def parse_html_to_messages(html_path: pathlib.Path, export_dir: pathlib.Path,
                           last_sender: dict, sender_map: dict):
    messages = []
    ...
    for div in soup.find_all("div", class_="message"):
        ...
        sender_el = body.find("div", class_="from_name", recursive=False) if body else None
        if sender_el:
        ...
        if is_service:
            ...
            service_text = body.get_text(strip=True) if body else ""
            low_text = service_text.lower()
            msg = {
                "id": msg_id, "type": "service",
                "date": dt, "date_unixtime": dt_unixtime,
            ...
            if m:
                ...
                messages.append(msg)
                continue
            a = body.find("a", onclick=True) if body else None
        ...
        if contact_div:
            ...
            msg["poll"] = {
                "question": question, "closed": False,
                "total_voters": total_voters, "answers": answers
            }
            messages.append(msg)
            continue
        ...
        if text_div:
            ...
            if has_formatting:
                ...
                text_list.append("")  # обязательный пустой элемент
                msg["text"] = text_list
...
def convert(html_file, output_file, export_dir, chat_name, chat_id, sender_map: dict):
    last_sender = {}
    msgs = parse_html_to_messages(html_file, export_dir, last_sender, sender_map)
...
def convert_html_export(
    ...
    """
    Главная точка входа для GUI: конвертирует весь HTML-бэкап в JSON.

    Args:
        export_dir: папка с messages.html и медиафайлами
        chat_id: числовой ID чата
        sender_map: {"Имя в бэкапе": "user_id"}
        log_callback: (message, level)
        progress_callback: (progress 0.0–1.0)
    """
    try:
        if log_callback:
            log_callback(f"Начало конвертации: {export_dir}", "INFO")

        messages_html = export_dir / "messages.html"
        if not messages_html.exists():
            raise FileNotFoundError(f"Файл messages.html не найден в {export_dir}")

        soup = BeautifulSoup(messages_html.read_text(encoding="utf-8"), "html.parser")
        header = soup.select_one(".page_header .text.bold")
        chat_name = header.get_text(strip=True) if header else export_dir.name

        if log_callback:
            log_callback(f"Название чата: {chat_name}", "INFO")

        html_files = sorted(export_dir.glob("messages*.html"))
        total = len(html_files)
        if log_callback:
            log_callback(f"Найдено HTML-файлов для конвертации: {total}", "INFO")

        for idx, html in enumerate(html_files):
            out = html.with_suffix(".json")
            convert(html, out, export_dir, chat_name, chat_id, sender_map)
            if log_callback:
                log_callback(f"✅ {html.name} → {out.name}", "SUCCESS")
            if progress_callback:
                progress_callback((idx + 1) / total)

        if log_callback:
            log_callback(f"✅ Конвертация завершена! Обработано файлов: {total}", "SUCCESS")
        return True

    except Exception as e:
        if log_callback:
            log_callback(f"❌ Ошибка конвертации: {str(e)}", "ERROR")
        return False